...
public:
    ...
    */
    ComposedPreconditioner(const boost::shared_ptr<Epetra_Comm> comm );
...
template <typename Operator>
ComposedPreconditioner<Operator>::ComposedPreconditioner(const boost::shared_ptr<Epetra_Comm> comm)
    :
    M_P(),
    M_Inverse(),
    M_Transpose(),
...
template <typename Operator>
const boost::shared_ptr<Epetra_Comm> ComposedPreconditioner<Operator>::
getCommPtr() const
{
    return( M_displayer.comm());
}
