...
//#include <life/lifesolver/one.hpp>
...
enum OneDBCStringValue { OneDBCLeftBoundary,
                         OneDBCRightBoundary,
                         OneDBCW1,
                         OneDBCW2,
                         OneDBCA,
...
typedef DataOneDModel::mesh_raw_type RegionMesh;
//! Forward declaration
...
public:
    ...
    Reimann( const FESpace<RegionMesh, EpetraMap>&   fespace,
             const FLUX&                             fluxFun,
...
protected:
    ...
    const FESpace<RegionMesh, EpetraMap>&    M_FESpace;
    //! Reference to the mesh
    //const RegionMesh& _M_mesh;
...
template< class FLUX >
Reimann<FLUX>::Reimann( const FESpace<RegionMesh, EpetraMap>& fespace,
                        const FLUX&                           fluxFun,
                        const std::vector<vector_type>&       U_thistime,
                        const std::string&                    border,
                        const std::string&                    var ):
    ...
    _M_dimDof                      ( fespace.dof().numTotalDof() ),
    _M_U_boundary                  ( 2 ),
    _M_W_boundary                  ( 2 ),
    _M_border                      ( border ),
    _M_var                         ( var )
{
    Debug( 6315 ) << "[OneDBCFUnction] Reimann \n";
...
{
public:
    Compatibility( const FESpace<RegionMesh, EpetraMap>& fespace,
                   const FLUX&                           fluxFun,
...
Compatibility<FLUX, SOURCE>::Compatibility( const FESpace<RegionMesh, EpetraMap>& fespace,
                                            ...
                                            const std::string&                    border,
                                            const std::string&                    var ):
    Reimann<FLUX>   ( fespace, fluxFun, U_thistime, /*W_thistime,*/ border, var),
    ...
    _M_U_internalBd ( 2 )
{
    ...
    switch( this->_M_oneDBCFunctionsMapStringValues[this->_M_border] )
        ...
        case OneDBCLeftBoundary:
            ...
            _M_boundaryEdge        = this->M_FESpace.mesh()->edgeList(1);
            _M_boundaryPoint       = _M_boundaryEdge.point(1).x();
            _M_internalBdPoint     = _M_boundaryEdge.point(2).x();
            break;
        case OneDBCRightBoundary:
            _M_internalBoundaryDof = this->_M_boundaryDof - 1;
            _M_boundaryEdge        = this->M_FESpace.mesh()->edgeList(_M_nb_elem);
            _M_boundaryPoint       = _M_boundaryEdge.point(2).x();
            _M_internalBdPoint     = _M_boundaryEdge.point(1).x();
            break;
        default:
