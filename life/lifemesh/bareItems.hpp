...
I need to identify edges and give them a unique ID. Yet, I may not want
...
makeBareEdge(ID const i, ID  const j){
  if(i<j)
    {
      return pair<BareEdge,bool>(BareEdge(i,j),true);
    }
  else
    {
      return pair<BareEdge,bool>(BareEdge(j,i),false);;
    }
}
...
class BareItemsHandler: public map<BareItem,ID,cmpBareItem<BareItem> >{
 public:
  typedef  map<BareItem,UInt,cmpBareItem<BareItem> > container;
  typedef  typename container::iterator iterator;
  typedef  typename container::const_iterator const_iterator;
  typedef  pair<const BareItem, UInt> value_type;
  ...
  pair<ID,bool> addIfNotThere(BareItem const &); //!< if not there adds it, the item ID is autogenerated
  pair<ID,bool> addIfNotThere(BareItem const &, const ID id); //!<if not there adds it, and sets ID id
...
template<typename BareItem>
inline
ID getId(pair<BareItem,ID> const & i){
     return i.second;
}
...
template<typename BareItem>
inline
BareItem getItem(pair<BareItem,ID> const & i){
     return i.first;
}
...
BareItemsHandler<BareItem>::addIfNotThere(const BareItem & s){
  pair<BareItemsHandler<BareItem>::iterator,bool> i(insert(make_pair(s,_idCount+1)));
  if (i.second) ++_idCount;
  return make_pair((i.first)->second,i.second);
}
...
BareItemsHandler<BareItem>::addIfNotThere(const BareItem & s, const ID id ){
  pair<BareItemsHandler<BareItem>::iterator,bool> i(insert(make_pair(s,id)));
  (i.first)->second=id; // Set new id in any case.
  return make_pair(id,i.second); // for consistency with other version.
}
