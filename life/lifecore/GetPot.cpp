            ...
            if ( *p == '=' )
            {
                ...
                *o = '\0';
                ...
                if ( Var == 0 )
                    ...
                    ( const_cast<GetPot::variable*>( Var ) ) ->take( p );
                *o = '=';
                break;
            }
        }
...
GetPot::read_in_stream( std::istream& istr )
{
    ...
    while ( istr )
    {
        ...
        int tmpfix = Token[ 0 ];
        if ( Token.length() == 0 || tmpfix == EOF )
            break;
#else

        if ( Token.length() == 0 ||
                std::char_traits<char>::eq_int_type( Token[ 0 ], std::char_traits<char>::eof() ) )
            break;
#endif
...
GetPot::DBE_expand( const std::string expr )
{
                ...
                expr.substr( 0, 2 ) == "<=" || expr[ 0 ] == '>' || expr[ 0 ] == '<' ) )
    {
        ...
        StringVector a;
        if ( op == GT || op == LT )
            a = DBE_get_expr_list( expr.substr( 1 ), 2 );
        else
            a = DBE_get_expr_list( expr.substr( 2 ), 2 );
