...
class FromTo{ public:
  int from,to;
  FromTo(int i,int j):from(i),to(j) {K_assert(i<j);} 
 };
...
//  SubArray(char  nn): n(-1),step(1),start(0) {}  
 explicit SubArray(int nn,int sta=0,int s=1): n(nn),step(s),start(sta) {}
  ...
  int end()  const  { return start+ step*n;}
  int last() const  { return start+ step*(n-1);}
  int len1() const  { return step*(n-1);}  
};
...
class ShapeOfArray{ protected:
  ...
  ShapeOfArray(const ShapeOfArray &old,const SubArray &sub) 
         : n(sub.n),step(old.step*sub.step),next(old.next)
          { K_assert((sub.last())*old.step <= old.last());} // a constructor 
          
  ShapeOfArray(const ShapeOfArray &old,int stepo,int start) 
         : n(old.n-start),step(old.step*stepo),next(old.next) 
          { K_assert(n>=0);}        
          
  int end()  const      { return n*step;}
  int last()      const      { return (n-1)*step;}
  int constant()  const { return step==0;}
  int index(int k) const { K_assert( (k>=0) && ( (k <n) || !step) );
           return step*k;}
  ShapeOfArray operator*(int stepp) const {return  ShapeOfArray(n,step*stepp,next);}  
  bool SameShape(const ShapeOfArray & a) const 
          { return  !step || !a.step || a.n == n ;} 
  int  N(const ShapeOfArray & a) { return step ? n : a.n;} // size of 2 shape 

  
// protected:
  int operator[](int k) const {  K_assert( (k>=0) && ( (k <n) || !step) );
           return step*k;}     
...
inline int N(const ShapeOfArray & a,const ShapeOfArray & b) 
           { K_assert(SameShape(a,b)); return  a.step ? a.n :  b.n ;}
...
public:
 ...
 private:
  ...
  KN_&  operator++(){K_assert(next>=0);v += next;return *this;} //    ++U
  KN_&  operator--(){K_assert(next>=0);v -= next;return *this;} //    --U
  KN_   operator++(int ){K_assert(next>=0); KN_ old=*this;v = v +next;return old;} // U++
  KN_   operator--(int ){K_assert(next>=0); KN_ old=*this;v = v -next;return old;} // U++
...
class KNMK_: public KN_<R> {
  ...
  int size() const { return shapei.n*shapej.n*shapek.n;}
...
class KNMKL_: public KN_<R> {
  ...
  int M() const {return shapej.n;}
  int K() const {return shapek.n;}
  int L() const {return shapel.n;}
  int size() const { return shapei.n*shapej.n*shapek.n*shapel.n;}
...
//  pas de tableau suivant
 ...
 KN_<R>  operator()(int i,const char ,int k, int l)  const  { // le tableau (i,.,k,l) 
        return KN_<R>(*this,-1,shapej,shapei[i]+shapek[k]+shapel[l]);}

 KN_<R>  operator()(int i,int j,const char, int l )  const  { // le tableau (i,j,.,l) 
        return KN_<R>(*this,-1,shapek,shapei[i]+shapej[j]+shapel[l]);}

 KN_<R>  operator()(int i,int j,int k, const char )  const  { // le tableau (i,j,k,.) 
...
//  ---------------------
template<class R>
class KNMKL: public KNMKL_<R>{ public:
...
class Add_KN_{public: 
  ...
  Add_KN_(const KN_<const_R> & aa,const KN_<const_R> & bb) 
     : a(aa),b(bb)  { K_assert(SameShape(a,b));}
 };  
...
class Add_Mulc_KN_ { public:
  ...
  Add_Mulc_KN_(const Mulc_KN_<R> & aa,const Mulc_KN_<R> & bb)  
        : a(aa.a),b(bb.a),ca(aa.b),cb(bb.b) { K_assert(SameShape(a,b));}
  Add_Mulc_KN_(const Mulc_KN_<R> & aa,const KN_<const_R> & bb,const R cbb) 
        : a(aa.a),b(bb),ca(aa.b),cb(cbb)  { K_assert(SameShape(a,b));}
  Add_Mulc_KN_(const KN_<const_R> & aa,const R caa,const KN_<const_R> & bb,const R cbb) 
        : a(aa),b(bb),ca(caa),cb(cbb) { K_assert(SameShape(a,b));}
 };  
...
class Mul_KNM_KN_ { public:
  ...
  Mul_KNM_KN_(const  KNM_<const_R> & aa,const KN_<const_R> & bb)  
        : A(aa),b(bb) {K_assert(SameShape(A.shapej,b));} 
};
...
template<class R> inline int SameAdress(const KN_<R> &a, const KN_<R> &b) { return &a[0]==&b[0];}
