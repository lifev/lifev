        ...
        public StructuralMaterial<Mesh>
{
  ...
  */
  void setup(const boost::shared_ptr< FESpace<Mesh, MapEpetra> >& dFESpace,
    ...
    */
    void computeKinematicsVariables( const ElemVec& dk_loc );
...
template <typename Mesh>
void ExponentialMaterialNonLinear<Mesh>::computeLinearStiffMatrix(dataPtr_Type& dataMaterial)
{
...
void ExponentialMaterialNonLinear<Mesh>::updateJacobianMatrix( const vector_Type& disp,
                                                              const dataPtr_Type& dataMaterial,
                                                              const displayerPtr_Type& displayer cd l)
{

    std::cout << std::endl;
    std::cout << "*********************************" << std::endl;
    displayer->leaderPrint("   Non-Linear S-  Using the Stiffness Matrix in UpdateJacobian");
    std::cout << std::endl;
    std::cout << "*********************************" << std::endl;
...
template <typename Mesh>
void ExponentialMaterialNonLinear<Mesh>::updateNonLinearJacobianMatrix( matrixPtr_Type& 	stiff,
                                                                       const vector_Type& 	disp,
                                                                       const dataPtr_Type& 	dataMaterial,
                                                                       const displayerPtr_Type& displayer )
{
	...
	for ( UInt i = 0; i < this->M_FESpace->mesh()->numVolumes(); ++i )
		...
		Real gamma  = dataMaterial->gamma(marker);
        	...
        	for ( UInt iNode = 0; iNode < ( UInt ) this->M_FESpace->fe().nbFEDof; iNode++ )
        	{
        		...
        		{
        	        	UInt ig = this->M_FESpace->dof().localToGlobal( eleID, iloc ) + iComp*this->M_FESpace->dim() + this->M_offset;
		...
		*/	
		...
		stiff_Jac_Pvol_1term( bulk, (*M_CofFk), (*M_Jack), *this->M_elmatK, this->M_FESpace->fe() );
	
		//! 2. Stiffness matrix: int { 1/2 * bulk * ( 1/J- 1 - log(J)/J^2 ) * ( CofF1 [\nabla \delta]^t CofF ) : \nabla v }
		stiff_Jac_Pvol_2term( bulk, (*M_CofFk), (*M_Jack), *this->M_elmatK, this->M_FESpace->fe() );  		
	    
	    	//! ISOCHORIC PART
		//! 1. Stiffness matrix : int { - 2/3 * zeta * alpha * J^(-5/3) * exp( gamma*( Ic_iso - 3) )* ( 1. + coefExp * Ic_iso )
		//!                      *( CofF : \nabla \delta ) ( F : \nabla \v ) }
	  	stiff_Jac_P1iso_Exp_1term( alpha, gamma, (*M_CofFk), (*M_Fk), (*M_Jack), (*M_trCisok), *this->M_elmatK, this->M_FESpace->fe() );
	    
	   	//! 2. Stiffness matrix : int { 2 * zeta * alpha * gamma * J^(-4/3) * exp( gamma*( Ic_iso - 3) ) *
		//!			 ( F : \nabla \delta ) ( F : \nabla \v ) }
	   	stiff_Jac_P1iso_Exp_2term( alpha * gamma, gamma, (*M_Fk), (*M_Jack), (*M_trCisok), *this->M_elmatK, this->M_FESpace->fe() );
	    
	   	//! 3. Stiffness matrix : int { 2.0/9.0 * zeta * alpha * J^-2 * Ic_iso * exp( gamma*( Ic_iso - 3) )* 
		//!                       ( 1. + gamma * Ic_iso )( CofF : \nabla \delta ) ( CofF : \nabla \v )}
	 	stiff_Jac_P1iso_Exp_3term( alpha, gamma, (*M_CofFk), (*M_Jack), (*M_trCisok), *this->M_elmatK, this->M_FESpace->fe() );
	    
	  	//! 4. Stiffness matrix : int { -2.0/3.0 * zeta * alpha * J^(-5/3) * exp( gamma*( Ic_iso - 3) )
		//!                    * ( 1. + gamma * Ic_iso )( F : \nabla \delta ) ( CofF : \nabla \v ) }
	  	stiff_Jac_P1iso_Exp_4term( alpha, gamma, (*M_CofFk), (*M_Fk), (*M_Jack), (*M_trCisok), *this->M_elmatK, this->M_FESpace->fe() );
	    
	  	//! 5. Stiffness matrix : int { zeta * alpha * J^(-2/3) * exp( gamma*( Ic_iso - 3)) (\nabla \delta: \nabla \v)}
	   	stiff_Jac_P1iso_Exp_5term( alpha, gamma, (*M_Jack), (*M_trCisok), *this->M_elmatK, this->M_FESpace->fe() ); 

	  	//! 6. Stiffness matrix : int { 1.0/3.0 * zeta * alpha * J^(-2) * Ic_iso *  exp(gamma*( Ic_iso - 3)) *  
		//!                       (CofF [\nabla \delta]^t CofF ) : \nabla \v  }
	   	stiff_Jac_P1iso_Exp_6term( alpha, gamma, (*M_CofFk), (*M_Jack), (*M_trCisok), *this->M_elmatK, this->M_FESpace->fe() );
...
void ExponentialMaterialNonLinear<Mesh>::computeStiffness( const vector_Type& disp,
						          ...
						          const displayerPtr_Type& displayer )
{
     this->computeVector( disp, factor, dataMaterial, displayer );
}


template <typename Mesh>
void ExponentialMaterialNonLinear<Mesh>::computeVector( const vector_Type& disp,
						       Real factor,
						       const dataPtr_Type& dataMaterial,
						       const displayerPtr_Type& displayer )
{
    std::cout << std::endl;
    std::cout << "*********************************" << std::endl;
    displayer->leaderPrint("   Non-Linear S-  Computing the nonlinear stiffness vector for Exponential material\n");
    std::cout << std::endl;
    std::cout << "*********************************" << std::endl;
     ...
     */ 
     source_P1iso_Exp( alpha, gamma, (*M_CofFk), (*M_Fk), (*M_Jack), (*M_trCisok), *this->M_elvecK, this->M_FESpace->fe());
		...
		from updateSystem(matrix_ptrtype& mat_stiff, vector_ptr_type& vec_stiff) 
        	*/
		assembleVector( *vec_stiff, *this->M_elvecK, this->M_FESpace->fe(), this->M_FESpace->dof(), ic, this->M_offset +  ic*totalDof );
		for ( UInt jc = 0; jc < nDimensions; jc++ )
		{            
			/*! 
		 	M_elmatK is assemble into *mat_stiff matrix that is recall
		 	from updateSystem(matrix_ptrtype& mat_stiff, vector_ptr_type& vec_stiff) 
			*/
        		assembleMatrix( *mat_stiff, *this->M_elmatK, this->M_FESpace->fe(), this->M_FESpace->dof(), ic, jc, this->M_offset +  ic*totalDof, this->M_offset +  jc*totalDof);
		}
     	}
     }
}
...
void StructuralSolver<Mesh, SolverType>::computeKinematicsVariables( const ElemVec& dk_loc )
{
	...
	for ( int icoor = 0; icoor < nDimensions; icoor++ )
		...
		for ( int jcoor = 0; jcoor < nDimensions; jcoor++ )
			{
			s = 0.0;
			for ( int i = 0; i < this->M_FESpace->fe().nbNode; i++ )
				{
				s += this->M_FESpace->fe().phiDer( i, jcoor, ig ) * dk_loc[ i + icoor * this->M_FESpace->fe().nbNode ]; // \grad u^k at a quadrature point
				}
...
namespace
{
static bool registerNH = StructuralMaterial<LifeV::RegionMesh3D<LinearTetra> >::StructureMaterialFactory::instance().registerProduct( "neoHookean", &createExponentialMaterialNonLinear<LifeV::RegionMesh3D<LinearTetra> > );
}
