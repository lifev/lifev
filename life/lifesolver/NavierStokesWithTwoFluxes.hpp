...
NavierStokesWithTwoFluxes<NSSolver>::solve(int label0, int label1)
{
    ...
    PhysVectUnknown<Vector> pns1(_M_solver->pDof().numTotalDof());
    PhysVectUnknown<Vector> pnso1(_M_solver->pDof().numTotalDof());
    PhysVectUnknown<Vector> pnso2(_M_solver->pDof().numTotalDof()); 
