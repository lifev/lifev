...
public:
    ...
    */
    ...
    /*!
      ...
      \param time present time
    */
    //! and pass as previous velocity the velocity field _u_in (for flux problem) - 11/08/04 VC
    void timeAdvance(const Function source, const Real& time, PhysVectUnknown<Vector> _u_in);

    //! Update convective term, bc treatment and solve the linearized ns system
    void iterate( const Real& time );

    //! Update convective term, bc treatment, solve the linearized ns system and return the velocity field to pass as previous velocity to another NS (for flux problem) - 11/08/04 VC
    PhysVectUnknown<Vector> iterate(const Real& time, int flag);
...
template <typename Mesh>
void NavierStokesSolverPC<Mesh>::
timeAdvance( const Function source, const Real& time , PhysVectUnknown<Vector> _u_in )
{
    ...
    // *******************************************************
    //_f_u += _M_u * this->_bdf.bdf_u().time_der(); //_M_u is the mass matrix divided by the time step
    _f_u += _M_u * _u_in; //compute the term M/Dt*u with u a given velocity field
    chrono.stop();
    std::cout << "done in " << chrono.diff() << " s." << std::endl;
}
...
template <typename Mesh>
PhysVectUnknown<Vector> NavierStokesSolverPC<Mesh>::
iterate( const Real& time, int flag )
{
