    ...
    if (flag-16>=0)//coupling the mesh in FSI
    {
        ...
        for ( UInt dim = 0; dim < nDimensions; ++dim )
        {
            for ( ITrow = locDofMap.begin(); ITrow != locDofMap.end() ; ++ITrow )
            {
                ...
                {
                    bigMatrix->addToCoefficient(solidFluidInterface + ITrow->first + dim*problem[2]->dof().numTotalDof(), offset[0] + ITrow->second + dim* problem[0]->dof().numTotalDof(), (-value)*rescaleFactor/**timeStep*//**1.e-2*//*scaling of the solid matrix*/ );
                }
            }
        }
        flag -= 16;
