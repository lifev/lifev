...
ZeroDimensionalData::ZeroDimensionalData():
    M_time                      (),
    M_outPutFormat              (OutPutFormat("20","12","   ",5000)),
    M_circuitData               (new ZeroDimensionalCircuitData)
{
}
ZeroDimensionalData::~ZeroDimensionalData()
{
...
ZeroDimensionalData::setup( const GetPot& dataFile, bcPtr_Type bc, const std::string   & section )
{
    ...
    std::string circuirDataFile = dataFile( (section + "/CircuitDataFile").data()                    , "./inputFilexx.dat" );
    GetPot dataFileCircuit( circuirDataFile );
    std::string circuitFile = dataFileCircuit("Files/InputFiles/CircuitFile"                         , "./circuitFilexx.dat" );
    std::string folder      = dataFileCircuit("Files/OutputFiles/Folder"                             , "outputxx" );//TODO creat folder output

    std::string voltageFile = folder + "/" + dataFileCircuit( "Files/OutputFiles/VoltageFile"         , "voltagexx.txt" );
    std::string currentFile = folder + "/" + dataFileCircuit( "Files/OutputFiles/CurrentFile"         , "currentxx.txt" );
    std::string balanceFile = folder + "/" + dataFileCircuit( "Files/OutputFiles/BalanceCurrentinNode", "balancexx.txt" );
    ...
    //Build the Circuit
    M_circuitData->buildCircuit(circuitFile.c_str(),bc);
    ...
    //Set Zero Initial Condition //TODO change to Generall Initial Condition
    initializeSolution();
...
void
ZeroDimensionalData::showMe() const
{
    M_circuitData->showMe();
}
void
ZeroDimensionalData::assignVaribleIndex()
{
    ...
    for (iterZeroDimensionalNodeUnknown_Type theUnknownNode = unKnownList->begin();
                        theUnknownNode != unKnownList->end(); theUnknownNode++) {
...
void
ZeroDimensionalData::writeHeaders(){
...
ZeroDimensionalData::saveSolution(){
    ptrVecZeroDimensionalElementPtr_Type elementList   = M_circuitData->Elements() ->elementList();
    M_outPutFormat.writeDataFormat(M_time->time(),M_currentFileStream,M_outPutFormat.space);

    //write current
    for (iterZeroDimensionalElement_Type theElement = elementList->begin(); theElement != elementList->end(); theElement++) {
        M_outPutFormat.writeDataFormat((*theElement)->current(),M_currentFileStream,M_outPutFormat.space);
    }

    //write voltage and current balance at each node
    M_outPutFormat.writeNewLine(M_currentFileStream);
    ptrVecZeroDimensionalNodePtr_Type nodeList     = M_circuitData->Nodes() ->nodeList();
    M_outPutFormat.writeDataFormat(M_time->time(),M_voltageFileStream,M_outPutFormat.space);
    M_outPutFormat.writeDataFormat(M_time->time(),M_balanceFileStream,M_outPutFormat.space);

    for (iterZeroDimensionalNode_Type theNode = nodeList->begin(); theNode != nodeList->end(); theNode++) {
        M_outPutFormat.writeDataFormat((*theNode)->voltage()       ,M_voltageFileStream,M_outPutFormat.space);
        M_outPutFormat.writeDataFormat((*theNode)->currentBalance(),M_balanceFileStream,M_outPutFormat.space);
    }
...
void
ZeroDimensionalData::showMeVariables()
{
    std::cout << "This MultiscaleModel0D::ShowVariables----------------------"<< std::endl;
    zeroDimensionalNodeSPtr_Type    Nodes       = M_circuitData->Nodes();
    zeroDimensionalElementSPtr_Type Elements = M_circuitData->Elements();

    for (iterZeroDimensionalNodeUnknown_Type theUnknownNode = Nodes->unknownNodeList()->begin();
