...
#define _darcySolverTransient_H_
...
staticCondensation ()
{
    ...
    dsyrk_( _param_L, _param_T, NBP, NBU, ONE_, B, NBU, ZERO_, this->M_BtB, NBP );
    ...
    /* Put in M_CtC the matrix C^T * L^{-T} * L^{-1} * C = C^T * A^{-1} * C
       ...
       For more details see http://www.netlib.org/slatec/lin/dsyrk.f */
    dsyrk_( _param_L, _param_T, NBL, NBU, ONE_, C, NBU, ZERO_, this->M_CtC, NBL );

    /* Put in M_BtC the matrix B^T * L^{-T} * L^{-1} * C = B^T * A^{-1} * C
       M_BtC fully stored.
       For more details see http://www.netlib.org/blas/dgemm.f */
    dgemm_( _param_T, _param_N, NBP, NBL, NBU, ONE_, B, NBU, C, NBU, ZERO_, this->M_BtC, NBP );
    ...
    /* Put in M_BtB the matrix LB and LB^T where LB and LB^T is the cholesky
       ...
       For more details see http://www.netlib.org/lapack/double/dpotrf.f  */
    dpotrf_( _param_L, NBP, this->M_BtB , NBP, INFO );
    ASSERT_PRE( !INFO[0],"Lapack factorization of BtB is not achieved." );

    /* Put in M_BtC the matrix LB^{-1} * M_BtC = LB^{-1} * B^T * A^{-1} * C.
       For more details see http://www.netlib.org/lapack/lapack-3.1.1/SRC/dtrtrs.f */
    dtrtrs_( _param_L, _param_N, _param_N, NBP, NBL, this->M_BtB, NBP, this->M_BtC, NBP, INFO );
    ...
    /* Put in M_CtC the matrix -M_CtC + M_BtC^T * M_BtC
       ...
       For more details see http://www.netlib.org/slatec/lin/dsyrk.f  */
    dsyrk_( _param_L, _param_T, NBL, NBP, ONE_, this->M_BtC, NBP, MINUSONE_, this->M_CtC, NBL );
    ...
    dgemv_( _param_N, NBP, NBP, ONE_, M_elmatMassPrimal.mat(), NBP, elvecPrimalOldLocal, INC1, ONE_, this->M_elvecSource, INC1 );

    /* Put in M_elvecSource the vector LB^{-1} * M_elvecSource = LB^{-1} *( M_elmatMassPrimal / \Delta t + F)
       For more details see http://www.netlib.org/lapack/lapack-3.1.1/SRC/dtrtrs.f */
    dtrtrs_( _param_L, _param_N, _param_N, NBP, NBRHS, this->M_BtB, NBP, this->M_elvecSource, NBP, INFO);
    ...
    /* Put in M_elvecHyb the vector
       ...
       For more details see http://www.netlib.org/blas/dgemm.f */
    dgemm_( _param_T, _param_N, NBL, NBRHS, NBP, ONE_, this->M_BtC, NBP, this->M_elvecSource, NBP, ZERO_, this->M_elvecHyb, NBL );
    ...
    this->symmetrizeMatrix( _param_L, NBL, this->M_CtC );
...
localComputePrimalAndDual ()
{
    ...
    /* Put in M_elvecSource the vector
       ...
       For more details see http://www.netlib.org/blas/dgemm.f */
    dgemm_( _param_N, _param_N, NBP, NBRHS, NBL, MINUSONE_, this->M_BtC, NBP, this->M_elvecHyb, NBL, ONE_, this->M_elvecSource, NBP );
    ...
    /* Put in M_elvecSource the vector LB^{-T} * M_elvecSource, where
       ...
       For more details see http://www.netlib.org/lapack/lapack-3.1.1/SRC/dtrtrs.f */
    dtrtrs_( _param_L, _param_T, _param_N, NBP, NBRHS, this->M_BtB, NBP, this->M_elvecSource, NBP, INFO );
    ...
    /* Put in M_elvecFlux the vector B * M_elvecSource = L^{-1} * B * primal_K
       For more details see http://www.netlib.org/slatec/lin/dgemv.f */
    dgemv_( _param_N, NBU, NBP, ONE_, B, NBU, this->M_elvecSource, INC1, ZERO_, this->M_elvecFlux, INC1 );

    /* Put in M_elvecFlux the vector
       - C * M_elvecHyb - M_elvecFlux = - L^{-1} * C * lambda_K - L^{-1} * B * primal_K
       For more details see http://www.netlib.org/slatec/lin/dgemv.f */
    dgemv_( _param_N, NBU, NBL, MINUSONE_, C, NBL, this->M_elvecHyb, INC1, MINUSONE_, this->M_elvecFlux, INC1 );
