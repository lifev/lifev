                                        ...
                                        const Real& coefficient)
{
    ...
    coupling->insertValueDiagonal( 1., fluidSolid , totalDofs);
    couplingMatrix(coupling, (*M_couplingFlags)[1]/*4*/, M_FESpace, M_offset, locDofMap, numerationInterface, timeStep, 2., coefficient);
    M_coupling.push_back(coupling);

    coupling.reset(new matrix_Type(*map, 0));
    coupling->insertValueDiagonal(1., M_offset[fluid], M_offset[solid] );
    coupling->insertValueDiagonal(-1,  fluidSolid, totalDofs);
    couplingMatrix(coupling, (*M_couplingFlags)[2]/*1*/,  M_FESpace, M_offset, locDofMap, numerationInterface, timeStep, 2., coefficient);
    M_coupling.push_back(coupling);

    coupling.reset(new matrix_Type(*map, 0));
    coupling->insertValueDiagonal( 1., M_offset[0], fluidSolid );
    coupling->insertValueDiagonal( 1., fluidSolid, totalDofs );
    couplingMatrix(coupling, (*M_couplingFlags)[3]/*2*/, M_FESpace, M_offset, locDofMap, numerationInterface, timeStep, 2., coefficient);
    M_coupling.push_back(coupling);
