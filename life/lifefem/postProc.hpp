...
/*!
  ...
  \brief class with all the methods for a post processing of the solution.
...
private:
    ...
    std::vector< SimpleVect<id_type> > _bdLtoG; // for each boundary face, it contains the numbering of the dof of the face
    std::vector< id_type > _fBdToIn; // it converts from a local numeration over the boundary faces on the global numeration of the mesh
    Mesh& _mesh;
...
PostProc<Mesh>::PostProc( Mesh& mesh, CurrentBdFE& feBd, const Dof& dof, UInt /*ncomp*/ = 1 ) : _mesh( mesh )
{
    ...
    SimpleVect<id_type> bdltg( nDofF );



    UInt iElAd, iVeEl, iFaEl, iEdEl;
    id_type lDof, gDof, auxDof, bDof = 1;
    std::vector<id_type>::iterator vidit;

    // ===================================================
    // Loop on boundary faces
    // ===================================================
    for ( id_type ibF = 1 ; ibF <= bdnF; ++ibF )
    {
        ...
        if ( nDofpV )
        {
            ...
            for ( id_type iVeFa = 1; iVeFa <= nFaceV; ++iVeFa )
            {
                ...
                for ( id_type l = 1; l <= nDofpV; ++l )
                {
                    ...
                    { // the gDof has been already inserted in the _fBdToIn vector
                        auxDof = ( id_type ) ( ( vidit - _fBdToIn.begin() ) ) + 1;
        ...
        {
            // loop on face edges
            for ( id_type iEdFa = 1; iEdFa <= nFaceV; ++iEdFa )
            {
...
void PostProc<Mesh>::set_area( CurrentBdFE& feBd, const Dof& dof )
{
    ...
    for ( id_type ibF = 1 ; ibF <= bdnF; ++ibF )
    {
        ...
        for ( id_type idofF = 1; idofF <= nDofF; ++idofF )
        {
            // global dof
...
void PostProc<Mesh>::show_bdLtoG()
{
    ...
    for ( std::vector<SimpleVect<id_type> >::iterator it1 = _bdLtoG.begin();it1<_bdLtoG.end();it1++ )
    {
        count++;
    ...
    for ( std::vector<id_type>::iterator it3 = _fBdToIn.begin();it3<_fBdToIn.end();it3++ )
    {
