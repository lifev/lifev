...
template<typename MatrixType, typename VectorType, typename MeshType, typename DataType>
  ...
  else { //! If BC is given under a functional form
...
template<typename VectorType, typename DataType>
  ...
  else {  //! If BC is given under a functionnal form
...
void bcNaturalManage(VectorType& b, const MeshType& mesh, const Dof& dof,  const BC_Base& BCb,
  ...
  */
    ...
    for (ID i=1; i<=BCb.list_size(); ++i) {
      ...
      // Loop on total Dof per Face
      for (ID idofF=1; idofF<= nDofF; ++idofF) {  //! fixed a possible BUG(??): it was the same variable : i for list and nDofF! (V. Martin)
