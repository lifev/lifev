...
public:
    ...
    */
    NonlinearBidomain( const data_type&          dataType,
...
NonlinearBidomain<Mesh, SolverType>::
NonlinearBidomain( const data_type&          dataType,
       FESpace<Mesh, EpetraMap>& FESpace,
       FESpace<Mesh, EpetraMap>& uFESpace,
       BCHandler&                BCh_u,
    ...
    M_bdf_u                  ( M_data.order_bdf()),
    M_CalCoef                ( M_data.CalCoef())
{
...
template<typename Mesh, typename SolverType>
void NonlinearBidomain<Mesh, SolverType>::setUp( const GetPot& dataFile )
{
    ...
    ASSERT(M_prec.get() != 0, "NonlinearBidomain : Preconditioner not set");
...
template<typename Mesh, typename SolverType>
void NonlinearBidomain<Mesh, SolverType>::buildSystem()
{
...
template<typename Mesh, typename SolverType>
void NonlinearBidomain<Mesh, SolverType>::
...
template<typename Mesh, typename SolverType>
void NonlinearBidomain<Mesh, SolverType>::PDEiterate( bchandler_raw_type& bch )
{
...
template<typename Mesh, typename SolverType>
void NonlinearBidomain<Mesh, SolverType>::solveSystem( matrix_ptrtype  matrFull,
                                           vector_type&    rhsFull )
{
    Chrono chrono;
...
template<typename Mesh, typename SolverType>
void NonlinearBidomain<Mesh, SolverType>::applyBoundaryConditions( matrix_type&        matrix,
                                                       vector_type&        rhs,
                                                       bchandler_raw_type& BCh )
{
