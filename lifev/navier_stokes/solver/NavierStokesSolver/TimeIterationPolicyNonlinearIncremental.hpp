...
#ifndef TIMEITERATIONPOLICYNONLINEARINCREMENTAL_HPP
#define TIMEITERATIONPOLICYNONLINEARINCREMENTAL_HPP
...
struct TimeIterationPolicyNonlinearIncremental : private AssemblyPolicy, public virtual SolverPolicy
{
protected:
...
void
TimeIterationPolicyNonlinearIncremental<mesh_Type, AssemblyPolicy, SolverPolicy>::
initTimeIteration ( Teuchos::ParameterList& list )
{
    ...
    M_deltaSolution.reset ( new vector_Type ( *M_solutionMap, Unique ) );
}
          ...
          const Real& currentTime )
{
    ...
    do
    {
        ...
        *M_rhs = 0.0;
        ...
        M_rhs->epetraVector().Update ( -1, Ax.epetraVector(), 1 );
        nonLinearResidual = M_rhs->norm2();
        ...
        if ( nonLinearResidual > M_nonLinearTolerance * normRhs )
        {
            ...
            *M_deltaSolution = 0.0;
            SolverPolicy::solve ( M_systemMatrix, M_rhs, M_deltaSolution );
            // int numIter = SolverPolicy::solve( M_systemMatrix, M_rhs, M_deltaSolution );
            // numIterSum += numIter; //

            if ( M_computeResidual )
            {
                vector_Type Ax ( M_deltaSolution->map() );
                vector_Type res ( *M_rhs );
                M_systemMatrix->matrixPtr()->Apply ( M_deltaSolution->epetraVector(), Ax.epetraVector() );
...
#endif /* TIMEITERATIONPOLICYNONLINEARINCREMENTAL_HPP */
