...
Real inflowTurek(const Real& t, const Real& x, const Real& y, const Real& z, const ID& i)
{
    if (i == 0)
    {
        Real H  		= 0.41;
        Real Um 		= 0.45;
        Real U  		= 16.0*Um*y*z*(H-y)*(H-z)/(H*H*H*H);
        Real inflowVel 	= 0.0;
        Real T_ramp    	= 2.0;

        if ( t <= T_ramp )
        {
        	inflowVel =	U/2.0*(1.0-std::cos(M_PI/T_ramp*t));
        }
        else
        {
        	inflowVel = U;
...
NavierStokes::run()
{
	...
	BCFunctionBase uInflow( inflowTurek );
    ...
    bcH.addBC( "Walls",         10, Essential, Full, 	uZero,   3);
    ...
    if(verbose && M_exportCoeff)
    {
        ...
        M_out << "% time / drag coefficient / lift coefficient \n" << std::flush;
    }
