...
main( int argc, char** argv )
{
    ...
    fespacePtr_Type uFESpace( new fespace_Type( localMeshPtr, uOrder, numDimensions, Comm ) );
    if (verbose) std::cout << "ok." << std::endl;

    if (verbose) std::cout << "Building the pressure FE space ... " << std::flush;
    fespacePtr_Type pFESpace( new fespace_Type( localMeshPtr, pOrder, 1, Comm ) );
    if (verbose) std::cout << "ok." << std::endl;
    ...
    *massMatrix *=0.0;
    ...
    // Update the BCHandler (internal data related to FE)
    fluxHandler.bcUpdate( *localMeshPtr, uFESpace->feBd(), uFESpace->dof());
