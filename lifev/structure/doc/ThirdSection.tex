\section{The StructuralSolver framework} In this section the \SSol
...
\begin{figure}[h!]  \centering
  ...
  \label{fig::design}
\end{figure}
\subsection{The \textit{StructuralSolver} class} This class is the
...
\begin{equation} \LE\left(\vectL^{n+1}\right)=0
  \label{eq::Root}
\end{equation} The \tPC{iterate} method contains the Newton method
...
\item \tPC{TimeAdvance}.
\end{itemize} The \tPC{StructuralMaterial} class defines the
...
\tPC{evalResidual()}. The second method provides to StructuralSolver
...
\end{itemize} In the case of linear elastic model, the Jacobian of
