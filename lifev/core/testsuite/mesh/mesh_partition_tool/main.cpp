...
// Build and partition the mesh
    ...
    if (verbose) std::cout << " -- Using Zoltan ... " << std::flush;
    meshCutterZoltan_Type meshPartZoltan(fullMeshPtr, Comm, meshParameters);
    if (! meshPartZoltan.success()) {
    	if (verbose) std::cout << "Zoltan partitioning failed." << std::endl;
    	return EXIT_FAILURE;
    }
    if (verbose) std::cout << " -- Using ParMETIS ... " << std::flush;
    meshCutterParMETIS_Type meshPartParMETIS(fullMeshPtr, Comm, meshParameters);
    if (! meshPartParMETIS.success()) {
    	if (verbose) std::cout << "ParMETIS partitioning failed." << std::endl;
    	return EXIT_FAILURE;
    }
    if (verbose) std::cout << " done ! " << std::endl;
...
// Build the FESpaces
	...
	// Error computation
		...
		if (verbose) std::cout << " -- Zoltan test successful! " << std::endl;
    }
	...
	// Error computation
		...
		if (verbose) std::cout << " -- ParMETIS test successful! " << std::endl;
    }
