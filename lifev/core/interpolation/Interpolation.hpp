...
public:
    ...
    void identifyNodes_known ( const FESpacePtr_Type& fespace );

    void identifyNodes_unknown ( const FESpacePtr_Type& fespace );
...
Interpolation::buildTableDofs_known ( const FESpacePtr_Type& fespace )
{
	...
	for ( int i = 0; i < numBFaces; ++i )
	{
		...
		if ( localToGlobalMapOnBFacet1.size() > 3 )
		{
			...
			FaceDof[3] = ( FaceDof[0] == FaceDof[1] ) ? FaceDof[0] : 1439;
			FaceDof[4] = ( FaceDof[1] == FaceDof[2] ) ? FaceDof[1] : 1439;
			FaceDof[5] = ( FaceDof[0] == FaceDof[2] ) ? FaceDof[0] : 1439;
		}
...
void
Interpolation::identifyNodes_known ( const FESpacePtr_Type& fespace )
{
	CurrentFEManifold feBd1 ( fespace->refFE().boundaryFE(), getGeometricMap ( *fespace->mesh() ).boundaryMap() );
	int numBFaces  = fespace->mesh()->numBFaces();
		...
		if ( fespace->mesh()->boundaryFacet ( i ).markerID() == M_flag )
		{
			...
			for (ID lDof1 = 0; lDof1 < localToGlobalMapOnBFacet1.size(); lDof1++)
			{
				if ( FaceDof[lDof1] == M_flag )
				{
					...
					if ( M_knownField->mapPtr()->map(Unique)->LID( static_cast<EpetraInt_Type> ( gDof1 ) ) >= 0 )
						M_GIdsKnownMesh.insert(gDof1);
				}
			}
		}
	}
...
void
Interpolation::identifyNodes_unknown ( const FESpacePtr_Type& fespace )
{
	CurrentFEManifold feBd1 ( fespace->refFE().boundaryFE(), getGeometricMap ( *fespace->mesh() ).boundaryMap() );
	int numBFaces  = fespace->mesh()->numBFaces();
	...
	for ( int i = 0; i < numBFaces; ++i )
	{
		if ( fespace->mesh()->boundaryFacet ( i ).markerID() == M_flag )
		{
			...
			for (ID lDof1 = 0; lDof1 < localToGlobalMapOnBFacet1.size(); lDof1++)
			{
				...
				{
					ID gDof1 = localToGlobalMapOnBFacet1[lDof1];
					if ( M_unknownField->mapPtr()->map(Unique)->LID( static_cast<EpetraInt_Type> ( gDof1 ) ) >= 0 )
						M_GIdsUnknownMesh.insert(gDof1);
				}
			}
		}
	}
