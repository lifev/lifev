...
void FSIMonolithicGI::buildSystem()
{
    super_Type::buildSystem();
    M_meshMotion->computeMatrix();
}

void FSIMonolithicGI::updateSystem()
{
    super_Type::updateSystem();
}

void FSIMonolithicGI::evalResidual( vector_Type& res, const vector_Type& disp, const UInt iter )
{
    ...
    if ( ( iter == 0 ) || !M_data->dataFluid()->isSemiImplicit() )
    {
        ...
        *M_rhsFull = *M_rhs;
        ...
        if ( !M_monolithicMatrix->set() )
        {
            ...
            M_monolithicMatrix->coupler( M_monolithicMap,
                                         M_dofStructureToFluid->localDofMap(),
                                         M_numerationInterface,
                                         M_data->dataFluid()->dataTime()->timeStep(),
                                         M_solidTimeAdvance->coefficientFirstDerivative( 0 ),
                                         M_solid->rescaleFactor() );
            M_monolithicMatrix->coupler( M_monolithicMap,
                                         M_dofStructureToHarmonicExtension->localDofMap(),
                                         M_numerationInterface,
                                         M_data->dataFluid()->dataTime()->timeStep(),
                                         M_solidTimeAdvance->coefficientFirstDerivative( 0 ),
                                         M_solid->rescaleFactor(),
...
void FSIMonolithicGI::setupBlockPrec()
{
                    ...
                    && M_data->dataSolid()->solidType().compare( "neoHookean" ) ) )
    {
        ...
        *M_solidBlockPrec *= M_solid->rescaleFactor();
        ...
        M_monolithicMatrix->replace_matrix( M_solidBlockPrec,
                                            0 );
    }
    ...
    if ( M_precPtr->blockVector().size() < 3 )
    {
        ...
        M_precPtr->coupler( M_monolithicMap,
                            M_dofStructureToFluid/*HarmonicExtension*/->localDofMap(),
                            M_numerationInterface,
                            M_data->dataFluid()->dataTime()->timeStep(),
                            M_solidTimeAdvance->coefficientFirstDerivative( 0 ),
                            M_solid->rescaleFactor(),
