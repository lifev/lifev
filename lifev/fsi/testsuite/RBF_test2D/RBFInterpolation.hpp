...
namespace LifeV 
{
  ...
  public:
    ...
    void buildInterpolationOperator();

    //! Build the projection operator
    void buildProjectionOperator();
  ...
  template <typename Mesh>
  void RBFInterpolation<Mesh>::setupRBFData(vectorPtr_Type KnownField, vectorPtr_Type UnknownField /*, double radius*/)
  {
  ...
  {
    this->buildInterpolationOperator();
    this->buildProjectionOperator();
    this->buildRhs();
  }

  template <typename Mesh>
  void RBFInterpolation<Mesh>::buildInterpolationOperator()
  {
    ...
    // GhostHandler<mesh_Type> ghostObj( M_fullMeshKnown, M_localMeshKnown, M_knownField->mapPtr(), M_knownField->mapPtr()->commPtr() );
    // ghostObj.setUp();
    ...
    for(set<ID>::iterator it = M_GIdsKnownMesh.begin(); it != M_GIdsKnownMesh.end(); ++it)
      {
	GlobalID[k] = *it;
	//MatrixGraph[k] = ghostObj.createNodeNodeNeighborsMapWithinRadius(M_RBFRadius, GlobalID[k]);
    ...
    spy("PHI", M_interpolationOperator);
  ...
  template <typename Mesh>
  void RBFInterpolation<Mesh>::buildProjectionOperator()
  {
	...
	for (int j = 0; j <  M_fullMeshKnown->numVertices(); ++j)
	  {
		...
		if (d < d_min/*M_RBFRadius*/)
		  {
		    //MatrixGraph[k].insert(M_fullMeshKnown->point(j).id());
		    d_min = d;
  ...
  template <typename Mesh>
  bool RBFInterpolation<Mesh>::isInside(ID pointMarker, flagContainer_Type M_flags)
  {
    int check = 0;
    for(UInt i = 0; i < M_flags.size(); ++i)
      if(pointMarker == M_flags[i])
	++check;
