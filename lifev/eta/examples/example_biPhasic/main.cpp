...
Real volumeForce( const Real& t, const Real& /*x*/, const Real& /*y*/, const Real& /*z*/, const ID& i)
{
    ...
    if (i == 0){ return shakeRadius*omega*omega*cos(theta) - shakeRadius*alpha*sin(theta); }
    if (i == 1){ return -shakeRadius*omega*omega*sin(theta) - shakeRadius*alpha*cos(theta); }
    if (i == 2){ return -9.806; }
...
int main( int argc, char** argv )
{
    ...
    if (verbose) std::cout << "alpha : " << alphaBL << std::endl;


    Real exactVolume(M_PI*cylinderRadius*cylinderRadius*liquidHeight);
    if (verbose) std::cout << "Exact : " << exactVolume << std::endl;
...
if (verbose) std::cout << "Hello " << exactVolume << std::endl;
	...
	if (verbose) std::cout << " BL factor : " << alphaBL << std::endl;
...
if (verbose) std::cout << " Level set : " << lsFESpace->map().map(Unique)->NumGlobalElements() << std::endl << std::endl;
...
if (verbose) std::cout << " Loading time stuff " << std::endl;
...
if (verbose) std::cout << " From " << currentTime << " to " << finalTime << std::endl;
...
if (verbose) std::cout << "[Navier-Stokes] Boundary integral ... " << std::flush;
...
if (verbose) std::cout << "[Navier-Stokes] Lifting the pressure ... " << std::flush;
...
if (verbose) std::cout << "[Navier-Stokes] Applying boundary conditions ... " << std::flush;
         ...
         std::vector<UInt> compXY(2); compXY[0]=0; compXY[1]=1;
...
if (verbose) std::cout << "[LS advection] Assembling the matrix ... " << std::flush;
...
if (verbose) std::cout << "[LS advection] Closing the matrix ... " << std::flush;
...
if (verbose) std::cout << "[LS advection] Assembling the rhs ... " << std::flush;
...
if (verbose) std::cout << "[LS advection] Closing the rhs ... " << std::flush;
...
if (verbose) std::cout << "[LS advection] Solving the system " << std::endl;
...
if (verbose) std::cout << "[Hamilton-J.] Initializing ... " << std::flush;
...
if (verbose) std::cout << ChronoItem.diff() << " s" << std::endl;
         ...
         if (niter % HJeach == 0)
         {
             ...
             while( HJtime < HJfinalTime)
             {
                 ...
                 if (verbose) std::cout << std::endl;
                 if (verbose) std::cout << "[Hamilton-J.] Time :  " << HJtime <<  std::endl;
                 if (verbose) std::cout << "[Hamilton-J.] Building the matrix ... " << std::flush;
...
#define BETA ( eval(sign, value(ETlsFESpace,LSSolutionOld) ) * eval(normalization, grad(ETlsFESpace,HJSolutionOld) ) )
                 ...
                 if (verbose) std::cout << "[Hamilton-J.] Closing the matrix ... " << std::flush;
                 ...
                 if (verbose) std::cout << "[Hamilton-J.] Assembling the rhs " << std::flush;
                 ...
                 if (verbose) std::cout << "[Hamilton-J.] Closing the rhs ... " << std::flush;
                 ...
                 if (verbose) std::cout << "[Hamilton-J.] Solving the system ... " << std::flush;
                 ...
                 if (verbose) std::cout << "[Hamilton-J.] Update the solution ... " << std::flush;
...
if (verbose) std::cout << "[LS correction] Correction of the volume ... " << std::flush;
    ...
    ChronoItem.stop();
if (verbose) std::cout << ChronoItem.diff() << " s" << std::endl;
if (verbose) std::cout << "[LS correction]     Rel. vol. diff:  " << (exactVolume-globalVolume)/exactVolume << std::endl;
if (verbose) std::cout << "[LS correction]     Approx. area:  " << globalArea << std::endl;


if (verbose) std::cout << "[LS advection] Time advancing ... " << std::flush;
...
if (verbose) displayMotionParameters(currentTime);
