...
/*!
 *   @file
     @brief This file contains the definition of the EvaluationDot class.

     @date 06/2011
     @author Samuel Quinodoz <samuel.quinodoz@epfl.ch>
 */

#ifndef EVALUTATION_DOT_HPP
#define EVALUTATION_DOT_HPP


#include <lifev/core/LifeV.hpp>

#include <lifev/eta/array/OperationSmallDot.hpp>
...
//! Evaluation for the dot product between two other Evaluations
/*!
  @author Samuel Quinodoz <samuel.quinodoz@epfl.ch>

  This class aims at representing an dot product during the assembly
...
public:
	...
	typedef typename OperationSmallDot< Lreturn_Type, Rreturn_Type >::result_Type return_Type;
	...
	//! Copy constructor
	EvaluationDot(const EvaluationDot& eval)
	...
	explicit EvaluationDot(const ExpressionDot<ExpressionL,ExpressionR>& expression)
	...
	{
        EvaluationLType::display(out); out << " dot "; EvaluationRType::display(out);
    }
    ...
    {
        return M_evaluationL.value_q(q).dot(M_evaluationR.value_q(q));
    }

    //! Getter for the value for a vector
	return_Type value_qi(const UInt& q, const UInt& i) const
    {
        return M_evaluationL.value_qi(q,i).dot(M_evaluationR.value_qi(q,i));
    }

    //! Getter for the value for a matrix
	return_Type value_qij(const UInt& q, const UInt& i, const UInt& j) const
    {
        return M_evaluationL.value_qij(q,i,j).dot(M_evaluationR.value_qij(q,i,j));
    }
...
template< typename EvaluationLType, typename EvaluationRType>
const flag_Type EvaluationDot<EvaluationLType,EvaluationRType>::S_globalUpdateFlag
 = EvaluationLType::S_globalUpdateFlag | EvaluationRType::S_globalUpdateFlag;

template< typename EvaluationLType, typename EvaluationRType>
const flag_Type EvaluationDot<EvaluationLType,EvaluationRType>::S_testUpdateFlag
 = EvaluationLType::S_testUpdateFlag | EvaluationRType::S_testUpdateFlag;

template< typename EvaluationLType, typename EvaluationRType>
const flag_Type EvaluationDot<EvaluationLType,EvaluationRType>::S_solutionUpdateFlag
