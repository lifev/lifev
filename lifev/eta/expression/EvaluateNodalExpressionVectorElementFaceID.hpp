...
#ifndef EVALUATENODALEXPRESSION_VECTOR_FACE_ID_HPP
#define EVALUATENODALEXPRESSION_VECTOR_FACE_ID_HPP
...
EvaluateNodalExpressionVectorElementFaceID (const std::shared_ptr<MeshType>& mesh,
    ...
    :   M_mesh (mesh),
        ...
        M_solutionSpace (testSpace),
        M_evaluation (expression),

        M_globalCFE (4),
        M_testCFE (4),

        M_elementalVector (SolutionTestSpace::field_dim * testSpace->refFE().nbDof() )
{
    for (UInt i (0); i < 4; ++i)
    {
        M_globalCFE[i] = new ETCurrentBDFE<3> (geometricMapFromMesh<MeshType>()
                                               , M_quadratureBoundary.qr (i) );
        M_testCFE[i] = new ETCurrentFE<3, SolutionTestSpace::field_dim> (testSpace->refFE()
                                                                      , testSpace->geoMap()
                                                                      , M_quadratureBoundary.qr (i) );
    }
        ...
        M_elementalVector (integrator.M_elementalVector)
{
    for (UInt i (0); i < 4; ++i)
    {
        M_globalCFE[i] = new ETCurrentBDFE<3> (geometricMapFromMesh<MeshType>()
                                               , M_quadratureBoundary.qr (i) );
        M_testCFE[i] = new ETCurrentFE<3, SolutionTestSpace::field_dim> (M_solutionSpace->refFE()
...
addTo (VectorType& vec)
{
    ...
    UInt nbTestDof (M_solutionSpace->refFE().nbDof() );

    /*CurrentBoundaryFE origFE(M_solutionSpace->refFE().boundaryFE(),
                             M_solutionSpace->geoMap().boundaryMap(),
                             quadRuleTria3pt);*/
    ...
    for (UInt iFace (0); iFace < nbBoundaryFaces; ++iFace)
    {
        ...
        // Loop on the blocks
        for (UInt iblock (0); iblock < SolutionTestSpace::field_dim; ++iblock)
        {
            ...
            for (UInt i (0); i < nbTestDof; ++i)
            {
                ...
                (i + iblock * nbTestDof,
                 M_solutionSpace->dof().localToGlobalMap (adjacentElementID, i) + iblock * M_solutionSpace->dof().numTotalDof() );
            }
