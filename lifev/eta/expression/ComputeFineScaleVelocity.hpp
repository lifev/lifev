...
#ifndef COMPUTE_FINESCALEVELOCITY_ELEMENT_HPP
#define COMPUTE_FINESCALEVELOCITY_ELEMENT_HPP
...
compute (VectorType& vec)
{
    ...
    VectorSmall<3> elementalVelocities;
    ...
    for (UInt iElement (0); iElement < nbElements; ++iElement)
    {
    	...
    	elementalVelocities = M_evaluation.value_qi (0, 0);

    	for (UInt i (0); i < 3; ++i)
    	{
    		vec[M_mesh->element(iElement).id() + i * M_mesh->numGlobalElements() ] = elementalVelocities(i);
    		elementalVelocities(i) = 0.0;
    	}
    }
