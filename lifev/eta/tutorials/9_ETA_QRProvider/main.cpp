...
// ---------------------------------------------------------------
    ...
    if (verbose) std::cout << " -- Changing the behaviour of the QRProvider ... " << std::flush;

    if (verbose) QuadratureRuleProvider::setBehaviorNoPreciseExactness(QuadratureRuleProvider::WarningAndReturnSup );
