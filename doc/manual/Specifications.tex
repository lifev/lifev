\documentclass[a4paper]{lifestyle}


\RCS $Revision: 1.2 $ 
\RCS $Date: 2004-02-08 08:54:48 $ 
\RCS $State: Exp $

%% \lfoot[{\tiny $Id: Specifications.tex,v 1.2 2004-02-08 08:54:48 prudhomm Exp $}]
%% {{\tiny $Id: Specifications.tex,v 1.2 2004-02-08 08:54:48 prudhomm Exp $}}

\newcommand{\thelibrary}{\textsl{LIFE V}}

%\author{Luca Formaggia, J.-Friederic Gerbeau}
%%\title{FEM library \thelibrary}

\title{Life Developer Manual}
\author{Life Developers}
\date{\lifedraftdate}
...
(CVS).  Details will be found in chapter \ref{ch:organisation}. All
documents will contain the CVS \texttt{{\$}Id{\$}} keyword: the
sources within the initial comment lines, the \LaTeX documents in the
command \verb!\date!. The CVS \texttt{{\$}Log{\$}} keyword is instead
added at the end of all documents, to keep track of all changes.
...
public:
  ...
  static UInt eToP(UInt const _localEdge, UInt const _point);
   static UInt fToP(UInt const _localFace, UInt const _point);
};
\end{verbatim}
...
\texttt{nbPtsPerEdge} & Number of Points \textbf{internal} to an edge\\
\texttt{nbPtsPerFace} & Number of Points \textbf{internal} to a face\\
\texttt{nbPtsPerVolume} & Number of Points \textbf{internal} to the volume
(i.e.\  to the tetrahedron itself).\\
\texttt{eToP(i,j)} & $j$-th point on the $i$-the edge (numbering from 1)\\
\texttt{fToP(i,j)} & $j$-th point on the $i$-the face (numbering from 1) \\
\hline
\end{tabularx}

The quantity \texttt{nbPtsPerVertex} is not stored because always equal to $1$.
We may note that other pieces of information are immediately deduced. If we take
the example of the Quadratic Tetra we have that the total number of point on each 
face is \texttt{GeoBShape::numPoints} while that for the Edges is 
given by \texttt{GeoBShape::GeoBShape::numPoints}. The Points numbering follow the
general convention
\begin{itemize}
...
\begin{displaymath}
\text{\texttt{QuadraticHexa::numVertices +
QuadraticHexa::nbPtsPerEdge*QuadraticHexa::numEdges}}
\end{displaymath}
...
class GeoND {
 public:
  ...
  // UInt const nPoints() const { return GeoShape::numPoints; };
  // UInt const nVertices() const { return GeoShape::numVertices;};
...
public:
  GeoElement1D();
#ifdef TWODIM
  /* In the 2D case, we store the ids of the adjacent 2Delements.
     NOT THE POINTERS because we don't know the 2Delements type and 
     I don't want to complicate the template declaration */
   UInt ad_first()const {return _first;};
   UInt ad_second()const {return _second;};
   UInt & ad_first(){return _first;}
   UInt & ad_second(){return _second;}
#endif
};
...
public:
  ...
  static const UInt numLocalEdges=GeoShape::numVertices;
...
public:
  ...
  static  const UInt numLocalFaces=GeoShape::numFaces;
  static  const UInt numLocalEdges=numLocalFaces+GeoShape::numVertices-2;
};
\end{verbatim}
...
\paragraph{name} The \emph{name} of a boundary condition is what \emph{uniquely}
distinguish a \texttt{BC} obiect to another within a container.
That is, whithin a given \texttt{BC\_Handler} object there cannot be two
\texttt{BC} objects with the same \textit{name}.
Furthermore, the tests for equality of two boundary condition objects is done
by checking the name.
In figure \ref{fig:bccond} we illustrate (graphically) the heirarchy 
of the \texttt{BC} classes.
\begin{figure}[hb]
\includegraphics[width=0.7\linewidth]{BCCond}
\caption{The organization of the \texttt{BC} classes.\label{fig:bccond}}
\end{figure}

The name type is alaised to \texttt{BCName}. Depending on a compiler
DEFINE flag (called \texttt{INT\_BCNAME})
\texttt{BCName}\index{INT\_BCNAME} may be a STL \texttt{string}
(default) or simply an \texttt{int}. A special constant,
\texttt{nullBCName} indicates a name which is associated to a null
boundary condition. It defaults to the empty string or to zero,
depending on the use of the compiler flag \texttt{INT\_BCNAME}, as
illustrated in the following code snapshot.
...
public:
  ...
  T& asLeaf(T  * _t);
  virtual ~BC_Base(){}
  BC_Base(BCName const _n);
#ifndef INT_BCNAME
  BC_Base(char const * _n);
#endif
  BC_Base();
  BCName & name();
  BCName const & name()const;
  bool unset()const;
protected:
  BCName _name;
};
...
{
public:
  BC_Base_WL(BCName const _n):BC_Base(_n){};
#ifndef  INT_BCNAME
...
\begin{verbatim}
  ...
  bool erase_BC(BCName const  _n); /* Deletes entry with name _n
  
  inline bool  isThere(BCName const _n) const;
...
<typename BC=BC_Base>
BC &
addBC(BC_Handler & bh, BCName const _n)

template
<typename BC=BC_Base>
BC *
addBC_ptr(BC_Handler & bh, BCName const _n)
\end{verbatim}
...
{
public:
  Dirichlet(BCName _n): BC_Base(_n)
    {};
...
{
public:
  Neumann(BCName const  _n): BC_Base_WL(_n)
    {};
...
{
public:
  static const UInt nbCoor = BRS::nDim;/* For Compatibility
...
/*
|   #Prerequisites
|   BasisFct:nbFct must be less than equal Geoele::numPoints
*/
{
public:
  GeoMap();
...
public:
  ...
  static const UInt nbCoor   = BasRefSha::nDim;
...
\section{Documentation Rules} All classes and methods should have
