...
typedef DataOneDModel::Vec2D              Vec2D;
typedef DataOneDModel::ScalVec            ScalVec;
...
public:
    ...
    OneDBC( const std::vector<vector_type>& U_thistime,
            const FLUX&                     fluxFun,
            const Real&                     dimDof,
    ...
    inline Vec2D& matrixrow( const std::string& line )
...
protected:
    ...
    Vec2D _solveLinearSyst2x2(const Vec2D& line1,
                                                const Vec2D& line2,
    ...
    std::map<std::string, Vec2D>                 _M_matrixrow_at_line;
    ...
    //const ScalVec_vector& _M_U_thistime;
    const std::vector<vector_type>&              _M_U_thistime;
...
public:
    ...
    //OneDBCHandler( const ScalVec_vector& U_thistime,
    OneDBCHandler( const std::vector<vector_type>& U_thistime,
                   const FLUX&                     fluxFun,
                   const Real&                     dimDof );
    ...
    void setBC( OneDBCFunctionPointer const& funptr,
                std::string const& border, std::string const& line, std::string const& var,
                Vec2D const& matrixrow );

    template< class SOURCE >
    void setDefaultBC(const FESpace<Mesh, EpetraMap>& fespace,
...
template< class FLUX >
OneDBC<FLUX>::OneDBC( const std::vector<vector_type>&  U_thistime,
                      const FLUX&                      fluxFun,
                      const Real&                      dimDof,
                      const std::string&               side )
    :
    ...
    _M_fluxFun(fluxFun)
{
    ...
    (side == "left") ? _M_boundaryDof = 1 : _M_boundaryDof = dimDof;
    ...
    _M_matrixrow_at_line["first"]=Vec2D(2);
    _M_matrixrow_at_line["second"]=Vec2D(2);
...
template< class FLUX >
Vec2D OneDBC<FLUX>::Uboundary(const ScalVec& U1,
                                                const ScalVec& U2) const
{
    Vec2D Ubound(2);
...
template< class FLUX >
void OneDBC<FLUX>::compute_resBC_line( std::string line, Vec2D left_eigvec,
                                       Vec2D U, Vec2D W, Real& rhs )
{
...
template< class FLUX >
Vec2D
OneDBC<FLUX>::_solveLinearSyst2x2(const Vec2D& line1,
                                  const Vec2D& line2,
                                  const Vec2D& rhs2d) const
{
...
template< class FLUX >
OneDBCHandler<FLUX>::OneDBCHandler( const std::vector<vector_type>& U_thistime,
                                    const FLUX&                     fluxFun,
                                    const Real&                     dimDof ):
    _M_U_thistime(U_thistime),
    _M_fluxFun(fluxFun)
...
template< class FLUX >
template< class SOURCE >
void OneDBCHandler<FLUX>::setDefaultBC( const FESpace<Mesh, EpetraMap>& fespace,
                                        const SOURCE&                   sourceFun,
                                        const Real&                     dt )
{
    ...
    if(!_M_boundarybool["left"].operator[]("first"))
        {
            border = "left";
            var    = "W1";
            OneDBCFunctionPointer point ( new Reimann<FLUX>( fespace,
                                                             _M_fluxFun,
                                                             _M_U_thistime, //_M_W_thistime,
                                                             //                                                             "left", "W1" ) );
                                                             border, var) );
            Debug( 6311 ) << "[OneDBCHandler::setDefaultBC] left-first-W1 Invoking setBC.\n";
    ...
    if(!_M_boundarybool["left"].operator[]("second"))
        {
            border = "left";
            var    = "W2";
            OneDBCFunctionPointer point ( new Compatibility<FLUX, SOURCE>( fespace,
                                                                           _M_fluxFun,
                                                                           sourceFun,
                                                                           _M_U_thistime, //_M_W_thistime,
                                                                           dt,
                                                                           border, var) );
            Debug( 6311 ) << "[OneDBCHandler::setDefaultBC] left-second-W2 Invoking setBC.\n";
    ...
    if(!_M_boundarybool["right"]["first"])
        {
            ...
            OneDBCFunctionPointer point ( new Reimann<FLUX>( fespace,
                                                             ...
                                                             _M_U_thistime, //_M_W_thistime,
                                                             border, var) );
            Debug( 6311 ) << "[OneDBCHandler::setDefaultBC] right-first-W2 Invoking setBC.\n";
