...
public:
	...
	const modelsTypes& 			GetType() 					const { return M_type; }

	//! Get the flag i-flag of the model
	/*!
	 * \param id - id of the boundary flag
	 */
	const BCFlag& 				GetFlag( const UInt& id )	const { return M_flags[ id ]; }

	//! Get the flag vector of the model
	const std::vector<BCFlag>& 	GetFlags() 					const { return M_flags; }
...
protected:
    ...
    UInt 						flagsNumber()			const { return static_cast<UInt>( M_flags.size() ); }
