...
RythmosModelInterface::RythmosModelInterface(int numGlobalElements,
  ...
  M_standardMap(0),
  M_initialSolutionY(0),
  M_initialSolutionYp(0),
  M_circuitData(circuitData)
{
  M_comm = comm->Clone();
  (*M_comm).PrintInfo(std::cout);
  M_commSharedPtr.reset(M_comm);
  M_mapEpetraPtr.reset(new MapEpetra(M_numGlobalElements,M_commSharedPtr));
...
//updateCircuitData from Y and Yp
...
#endif
  M_circuitData->updateCircuitDataFromY(t,x,x_dot);
  M_circuitData->updateABC(*M_A,*M_B,*M_C);
  //calculate the sum
#ifdef HAVE_LIFEV_DEBUG
  ...
  M_A->matrixPtr()->Multiply1(false,*x_dot,*M_fA);
#ifdef HAVE_LIFEV_DEBUG
  M_A->matrixPtr()->Print(std::cout);
#endif

  M_B->matrixPtr()->Multiply1(false,*x,*M_fB);
#ifdef HAVE_LIFEV_DEBUG
  M_B->matrixPtr()->Print(std::cout);
#endif
  for (int i = 0; i < M_numGlobalElements;i++){
