...
enum couplingsTypes{ BoundaryCondition, FluxPressure };
...
public:
	 ...
	 */
	void AddModel		( const PhysicalModel_ptr& model )					{ M_models.push_back( model ); }
	 ...
	 */
	void AddFlagID		( const UInt& flagID )								{ M_flags.push_back( M_models.back()->GetFlag( flagID ) ); }
	...
	//! Get the ID of the coupling
	const UInt& 				GetID() 			const { return M_ID; }

	//! Get the type of the coupling
	const couplingsTypes& 		GetType() 			const { return M_type; }

	//! Get the name of the coupling
	const std::string&			GetCouplingName() 	const { return M_couplingName; }
	...
	//! Update the values of the couplings variables
    virtual void UpdateCoupling( void ) = 0;
...
protected:
    ...
    inline UInt modelsNumber()		const { return static_cast<UInt>( M_models.size() ); }
