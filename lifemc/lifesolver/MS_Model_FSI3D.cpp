...
#ifdef DEBUG
    Debug( 8140 ) << "MS_Model_Fluid3D::MS_Model_Fluid3D() \n";
#endif
...
{
    //Global data time
    //M_solver->FSIOper()->dataFluid().setDataTime( PhysicalData->GetDataTime() );

    //Global physical quantities
    //M_solver->FSIOper()->dataFluid().setDensity( PhysicalData->GetFluidDensity() );
    //M_solver->FSIOper()->dataFluid().setViscosity( PhysicalData->GetFluidViscosity() );
...
MS_Model_FSI3D::SolveSystem()
{
    ...
    Real time = M_solver->FSIOper()->dataFluid().dataTime()->getTime()
              + M_solver->FSIOper()->dataFluid().dataTime()->getTimeStep();
