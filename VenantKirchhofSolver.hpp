...
//    void updateSystem( source_type const& source );
    ...
    void iterateMonolithic(vector_type& rhs, vector_type& sol, matrix_ptrtype prec, matrix_ptrtype robinCoupling); // used for monolithic
...
//! evaluates residual for newton interations
    ...
    void evalResidual(bchandler_raw_type & bcFluid, bchandler_raw_type & bcSolid, const vector_type& sol, const vector_ptrtype rhs, vector_type& res, bool diagonalScaling); // used for monolithic

    void evalResidual( const vector_type& sol/*, const vector_ptrtype rhs*/,  vector_type& res, bool diagonalScaling); // used for monolithic
    ...
    void    diagonalScale(vector_type&);
    ...
    const matrix_ptrtype getMatrixPtr() const {return M_matrFull;}// used for monolithic
    ...
    void setMatrix(){M_linearSolver.setMatrix(*M_matrFull);}
...
private:
    ...
    boost::shared_ptr<vector_type>                    M_disp;
    vector_type                    M_vel;
...
VenantKirchhofSolver( const data_type&          data,
    ...
    M_disp                       ( new vector_type(M_localMap)),
    M_vel                        ( M_localMap ),
...
//    M_linearSolver.setMatrix( M_jacobian );
    ...
    M_maxIterSolver = dataFile( "solid/solver/max_iter", 100);
    M_maxIterForReuse = dataFile( "solid/solver/max_iter_reuse", M_maxIterSolver*8/10);
    M_rescaleFactor = dataFile( "solid/prec/rescale_factor", 1.);
...
rescaleMatrices()
{
    *M_mass *=(M_data.timestep()*M_rescaleFactor);
    *M_linearStiff *= (M_data.timestep()*M_rescaleFactor);
    *M_massStiff *= (M_data.timestep()*M_rescaleFactor);
}

template <typename Mesh, typename SolverType> // for monolithic
void VenantKirchhofSolver<Mesh, SolverType>::
diagonalScale(vector_type& rhs)
{
    ...
    M_matrFull->getEpetraMatrix().LeftScale(diagonal);
    //prec->getEpetraMatrix().LeftScale(diagonal);
...
{
    matrix_ptrtype tmp;
    tmp.reset(new matrix_type(M_localMap));
    *tmp += *M_massStiff;
    *tmp += couplingMatrix;
    tmp->GlobalAssemble();
    //M_massStiff.reset(new matrix_type(*tmp));
    M_couplingMatrix.reset(new matrix_type(*tmp));
...
updateSystem( vector_type & rhsFluidCoupling )
{
    ...
    rhsFluidCoupling += *M_mass * _z ;
    rhsFluidCoupling -= *M_stiff*M_dispSolid;
...
template <typename Mesh, typename SolverType> // for monolithic
void VenantKirchhofSolver<Mesh, SolverType>::
evalResidual( bchandler_raw_type& bchFluid, bchandler_raw_type& bchSolid, const vector_type& sol, const vector_ptrtype rhs, vector_type& res, bool diagonalScaling)
{
    ...
    //    applyBoundaryConditions( *M_matrFull, rhsFullSolid, bchSolid, M_offset );
        if(M_offset)
            bchSolid.setOffset(M_offset);
        if ( !bchSolid.bdUpdateDone() )
            bchSolid.bdUpdate( *M_FESpace.mesh(), M_FESpace.feBd(), M_FESpace.dof() );

        bcManage( *M_matrFull, rhsFullSolid, *M_FESpace.mesh(), M_FESpace.dof(), bchSolid, M_FESpace.feBd(), 1.,
                  M_data.time() );
    ...
    //    M_rhs.reset(new vector_type(rhsFull));

    *M_rhs = rhsFull;
    //    M_rhs->GlobalAssemble();
    //    M_rhs->spy("rhs2");

    evalResidual(sol,/* M_rhs,*/ res, diagonalScaling);
    //    res.spy("resBefore");
    //    M_rhs=res;
}
...
template <typename Mesh, typename SolverType> // for monolithic
void VenantKirchhofSolver<Mesh, SolverType>::
iterateMonolithic(vector_type& rhs, vector_type& step, matrix_ptrtype prec, matrix_ptrtype robinCoupling)
{
    Chrono chrono;
    ...
    if(false && robinCoupling.get())
        {
            matrix_type tmpMatrix(*M_matrFull);
            tmpMatrix.GlobalAssemble();
            M_matrFull.reset(new matrix_type(M_localMap));
            EpetraExt::MatrixMatrix::Multiply( robinCoupling->getEpetraMatrix(), false, tmpMatrix.getEpetraMatrix(), false, M_matrFull->getEpetraMatrix());
    ...
    if  ( !M_reusePrec || M_resetPrec )
    {
        ...
        //vector_type useless(M_uFESpace->map());
        //matrix_ptrtype uselessMat( new matrix_type(*prec));
        //uselessMat->getEpetraMatrix().FillComplete(uselessMat->getEpetraMatrix().OperatorDomainMap(), *useless.getMap().getMap(Unique));
