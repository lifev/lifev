...
StabilizationIP<MESH, DOF>::StabilizationIP( const mesh_type mesh,
        ...
        M_mesh      ( mesh ),
        M_dof       ( dof ),
        M_fe1       ( refFE, getGeometricMap(*mesh), quadRule ),
        M_fe2       ( refFE, getGeometricMap(*mesh), quadRule ),
        M_feBd      ( feBd ),
...
//        << stab_norm << "\n\n\n\n" << std::endl;
    if (_verb)
        std::cout << "  s-  IP relative stabilization norm: " <<
...
//EDEB
    ...
    if (_verb)
        std::cout << "  s-  IP terms assembling: " <<
