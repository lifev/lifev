...
public NavierStokesAleHandler<Mesh> {
 ...
 private:
  ...
  // le deplacement a chaque pas de temps
  // PhysVectUnknown<Vector> _u_sub_it;
...
NavierStokesAleSolverPC(const GetPot& data_file, const RefFE& refFE_u, const RefFE& refFE_p, const QuadRule& Qr_u,
     ...
     // dep_fluid_interf(3*_mesh.numVertices()),
     //noeudFSI(_dim_u),  a reflechir
...
timeAdvance(const Function source, const Real& time) {
  ...
  //_disp_sub_it=_disp; // pour les sous-iterations
 // _u_sub_it=_u; // pour les sous-iterations
...
iterateLin(BC_Handler& BCh_du) {
  ...
  cout << "  maxnorm ( difference ) after BC= " <<  maxnorm( _f_duWithOutBC - _f_u)   << endl << flush; 
 ...
 // data_org assigned "by hands" while no parallel computation is performed
  ...
  // intermediate velocity computation   
  cout << "  o-  Solving system (i)... " << flush; 
  chrono.start();
  AZ_iterate(_du.giveVec(), _f_u.giveVec(), options_i,params_i, status_i,
  	     proc_config_i, C, prec_C, NULL);
  chrono.stop();
  cout << "done in " << chrono.diff() << " s." << endl << flush;
...
sdToMasterFSI() {
  ...
  static int ipass=0;
  force = new double[nbNoeudFSI*3]; 
  int nt = 3*nbNoeudFSI;
  double vref=1.;
  cout <<  "---> Fluid sends to coupler " << endl;
  for (int k=0 ; k<nbNoeudFSI ; ++k){
    for(int l=0;l<3;l++)
       force[3*k+l]=_residual_u[noeudFSI[k]-1+l*_dim_u]; // a modifier pour commencer de zero          
  }
  pvm_initsend(PvmDataDefault);
  pvm_pkint(&nt,1,1);
  pvm_pkdouble(force,nt,1);
  if(!ipass){
    pvm_pkdouble(&vref,1,1);
    ipass = 1;
  }
  pvm_send(masterId,120);
  delete[] force;
};
#endif
...
template<typename Mesh> 
void NavierStokesAleSolverPC<Mesh>::
rvFromMasterFSI(int msg) {
  // cout << "<-- Fluid receives from coupler\n" << flush;
  //cout << "--> " << nbNoeudFSI << endl << flush;
  //double dep_master[3*nbNoeudFSI]; 
  //dep_fluid_interf = new double[3*nbNoeudFSI]; // c'est pas la bonne taille
  //for(int i=0;i<nbNoeudFSI;i++)
  //  dep_master[3*i ]=dep_master[3*i+1]=dep_master[3*i+2]=0.;
  switch(msg){
  case 140:
    {
      cout << "Message 140\n" << flush;
      int nnifs,ntdlf;
      int* p_fnifs;
      pvm_recv(masterId,140);
      pvm_upkint(&nnifs,1,1);
      p_fnifs = new int[nnifs];
      pvm_upkint(&ntdlf,1,1);
      pvm_upkint(p_fnifs,nnifs,1);
      nbNoeudFSI= nnifs;
      noeudFSI = new int[nnifs];
      cout << "Noeuds FSI:" << noeudFSI << endl;
      for(int i=0;i<nnifs;i++) {
	noeudFSI[i] = p_fnifs[i]; 
      }
      //      cout << endl;
      cout << "<-- Fluid receives from coupler\n" << flush;
      cout << "--> " << nbNoeudFSI << endl << flush;
      dep_master = new double[3*nbNoeudFSI]; 
      for(int i=0;i<nbNoeudFSI;i++)
        dep_master[3*i ]=dep_master[3*i+1]=dep_master[3*i+2]=0.;
      cout << "ok 140.\n" << flush;
      break;
   ...
   case 110:
    { 
      ...
      //for(int i=0;i<nbNoeudFSI;i++)
      //  dep_master[3*i ]=dep_master[3*i+1]=dep_master[3*i+2]=0.;
      pvm_recv(masterId,110);
      pvm_upkint(&fsiStatus,1,1);
      cout << " le statut " << fsiStatus << endl << flush;
      if(fsiStatus>=0){
       	pvm_upkdouble(&_dt,1,1);
	pvm_upkint(&ntdlf,1,1);
	if(ntdlf != 3*nbNoeudFSI){
	  cout << "rv: ntdlf = " << ntdlf << " 3*nbNoeudFSI =" << 3*nbNoeudFSI
	       << endl << flush;
	  //exit(1);
	}
        cout << "avant de recevoir le depl de master" << endl << flush;
	pvm_upkdouble(dep_master,3*nbNoeudFSI,1);
        cout << "apres avoir recu le depl de master" << endl << flush;
	//for(int i=0;i<nbNoeudFSI;i++){
        //for(UInt icoord=0;icoord<3;icoord++){
        // dep_fluid_interf[noeudFSI[i] + _mesh.numVertices()*icoord]=dep_master[3*i + icoord];
        // cout << "dep_fluid_interf  "  << noeudFSI[i] + _mesh.numVertices()*icoord  << " est : "
        //      <<  dep_fluid_interf[noeudFSI[i] + _mesh.numVertices()*icoord]  << endl;   
	//}  
	//}
      }
    }// fin du case 110.
...
template<typename Mesh> 
void NavierStokesAleSolverPC<Mesh>::
dep_fluid_interf(Vector& analytic_disp){
  int dim_mesh = dofMesh().numTotalDof();
  for(int i=0;i<nbNoeudFSI;i++){
        for(UInt icoord=0;icoord<3;icoord++){
         analytic_disp[noeudFSI[i]- 1 + dim_mesh*icoord]=dep_master[3*i + icoord];
         //cout << "dep_fluid_interf  "  << noeudFSI[i] + _mesh.numVertices()*icoord  << " est : "
         //     << analytic_disp[noeudFSI[i] + _mesh.numVertices()*icoord]  << endl;   
	 }  
       }
