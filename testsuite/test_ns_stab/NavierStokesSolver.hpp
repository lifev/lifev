...
namespace LifeV
{
  ...
  */
  ...
  NavierStokesSolver( const GetPot& dataFile,
    ...
    NavierStokesHandler<Mesh>( dataFile, refFE, refFE, quadRule,
                               ...
                               bcHandler ),
    M_fullPattern( _dof_u, _mesh, nDimensions+1, this->patternType() ),
    M_uPattern( _dof_u, _mesh, nDimensions, this->patternType() ),
    ...
    M_rhsU( nDimensions*_dim_u ),
    M_rhsFull( ( nDimensions+1 )*_dim_u ), 
    M_rhsFullAux( ( nDimensions+1 )*_dim_u ),
    M_sol( ( nDimensions+1 )*_dim_u ),  
    M_res( ( nDimensions+1 )*_dim_u ),
    M_betaFct( 0 ),
	      ...
	      this->viscosity() )
  {
    ...
    std::cout << std::endl;
    std::cout << "O-  Velocity and pressure unknowns: " << _dim_u     << std::endl;
    ...
    for ( UInt iVol = 1; iVol <= _mesh.numVolumes(); iVol++ )
    {
        ...
        for ( UInt iComp = 0; iComp<nbCompU; iComp++ )
        {
	      ...
	      // stiffness
	      assemb_mat( M_matrStokes, M_elmatC, _fe_u, _dof_u, iComp, jComp );
	
	    if ( !M_steady )
	      assemb_mat( M_matrMass, M_elmatMass, _fe_u, _dof_u, iComp, iComp);
    ...
    for ( UInt iVol = 1; iVol<= _mesh.numVolumes(); ++iVol )
        ...
        for ( UInt iComp = 0; iComp<nbCompU; ++iComp )
	  {
	    ...
	    for (UInt ic = 0; ic < nbCompU; ++ic)
	      grad( ic, M_elvec, M_elmatC, _fe_u, _fe_u, iComp, iComp );
  ...
  */
  ...
  void NavierStokesSolver<Mesh>::initialize( const std::string & vname )
  {
    ...
    for ( UInt iDof = 0; iDof<_dim_u; ++iDof )
      {
	resfile >> M_sol[iDof+nDimensions*_dim_u  ]  ;
        _p[ iDof ] = M_sol[ iDof+nDimensions*_dim_u ];
      }
