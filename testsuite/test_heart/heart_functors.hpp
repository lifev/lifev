...
public:
    ...
    Real Iapp /*(const double& t,
		const double& x,
		const double& y,
		const double& z,
		const ID& i,
		const EntityFlag& ref)*/
		( const Real& x, const Real& y, const Real& z, const Real& t, const EntityFlag& ) const
		{
  ...
  Real IappZygote(const double& t,
	...
	double pi = acos(-1.0);
     ...
     /*
       ...
       Real stim_y[12];
       Real stim_z[12];
       stim_x[0]= 3.316424;  stim_y[0] =  31.496351; stim_z[0] = 5.799850;//APEX, node number: 80185 : 3.316424 31.496351 5.799850
       stim_x[1]= 1.864521;  stim_y[1] = 39.227596;  stim_z[1] = 4.989914;//Noeud atrio-ventriculaire  104969 : 1.864521 39.227596 4.989914  
       stim_x [2]=  4.456026; stim_y[2]=  31.445936; stim_z[2]= 3.588622 ; //  vertex   48172 : 4.456026 31.445936 3.588622   ref 0
       stim_x[3] = 6.104578; stim_y[3]= 36.508255; stim_z[3]= 6.916658 ; // vertex   90011 : 6.104578 36.508255 6.916658   ref 0
       stim_x[4] = -2.322989; stim_y[4]= 36.513706; stim_z[4] = 6.998336  ; //vertex   87199 : -2.322989 36.513706 6.998336   ref 0
       stim_x[5] =4.455060; stim_y[5]= 38.209770; stim_z[5]= 0.731818   ;//  vertex   75285 : 4.455060 38.209770 0.731818   ref 0
       stim_x[6] =  0.744906; stim_y[6]; 33.562958 ;stim_z[6]=2.410753   ;//  vertex   80024 : 0.744906 33.562958 2.410753   ref 0
       stim_x[7] =-0.401082; stim_y[7]= 40.853840; stim_z[7]= 4.965571    ;//  vertex   48505 : -0.401082 40.853840 4.965571   ref 0
       stim_x[8] =  6.066005; stim_y[8]= 34.837322; stim_z[8]= 2.086808  ;//   vertex   54209 : 6.066005 34.837322 2.086808   ref 0
       stim_x[9] =4.938366; stim_y[9]= 38.498398;stim_z[9]= 0.959437    ;//  vertex   46343 : 4.938366 38.498398 0.959437   ref 0
       stim_x[10] = 6.042288; stim_y[10]= 34.131123; stim_z[10]= 2.392315   ;//  vertex   47897 : 6.042288 34.131123 2.392315   ref 0
     */
    ...
    /*    
	  if(G_Ventricular_Tachycardy ==1)
	  { 
	  if (((x-x_0)*(x-x_0) + (y-y_0)*(y-y_0) + (z-z_0)*(z-z_0)<=0.4) && ( (fmod(t, 1000*60/G_Vent_Tachycardy_freq)<=5) ) )
	  iapp=4; 
	  }
	  
	  if(G_Ventricular_Flutter == 1 )
	  { 
	  if (((x-x_0)*(x-x_0) + (y-y_0)*(y-y_0) + (z-z_0)*(z-z_0)<=0.4) && ( (fmod(t,1000*60/G_Vent_Flutter_freq)<=5) ) )
	  iapp=2; 
	  }
    */
   /*
    if(G_Ventricular_Fibrillation == 1)
       {
	 for (int i =0; i<  G_nb_fibrillation_sources; i++)
	   {
	     double xx   = G_fibrillation_sources[i][1];
	     double yy   = G_fibrillation_sources[i][2];
	     double zz   = G_fibrillation_sources[i][3];
	     double freq =G_fibrillation_sources[i][4];
	     if ( (fmod(t,1000*60/freq)<=5) && ( (x -xx)*(x- xx)+ (y- yy)*(y - yy)+(z-zz)*(z-zz) <=0.2)   )
	       iapp=2;
	   }
       }    
     */
