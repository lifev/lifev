...
namespace LifeV
{
  ...
  public:
    ...
    operFS(F& fluid, S& solid, BCHandler& bcHdp, BCHandler& sum_p, BCHandler& sum_dp, BCHandler& bcHdz);
    ...
    Vector _dispStruct, _disp_1, _disp_2;
    Vector _velo;
    Vector _dz, _a, _da;
    Vector _rhs_dz;
  ...
  template <typename F, typename S> 
  operFS<F,S>::operFS(F& fluid, S& solid, BCHandler& bcHdp, BCHandler& sum_p, BCHandler& sum_dp, BCHandler& bcHdz):
    _fluid(fluid),
    _solid(solid),
    _dispStruct( 3*_solid.dDof().numTotalDof() ), 
    _disp_1( 3*_solid.dDof().numTotalDof() ),
    _disp_2( 3*_solid.dDof().numTotalDof() ),
    _velo( 3*_solid.dDof().numTotalDof() ),
    _dz( 3*_solid.dDof().numTotalDof() ), 
    _a( 3*_solid.dDof().numTotalDof() ),  
  ...
  void  operFS<F,S>::eval(Vector& dispNew, Vector& velo, const Vector& disp, int status) {
    ...
    double dt = _fluid.timestep();
    double dti = 1.0/dt;
    double dti2 = 1.0 / (dt*dt);

    _a = - _fluid.density()* dti2 *(  disp - 2.0*_disp_1 +  _disp_2 );

    _fluid.iteratePressure(_time);
    _fluid.updateForce(_sum_p);

    _solid.setRecur(0);
    _solid.iterate();
 
    dispNew = _solid.disp();
    velo    = (dispNew - _disp_1 ) * dt;
  ...
  void  operFS<F,S>::solveLinearFluid() {
    ...
    _fluid.iteratePressureLin(_time,_bcHdp);
    _fluid.updateLinForce(_sum_dp);
  }
