...
(which implies that the matrix needs to be reassembled each time)
 ...
 Purpose: Test of Post Processing Class
...
int main()
{
    ...
    PostProc<RegionMesh3D<LinearTetra> > post_proc(aMesh,feBd,dof,1);
    post_proc.show_bdLtoG();

    post_proc.set_area(feBd,dof);
    post_proc.show_area();

    post_proc.set_normal(feBd,dof);
    post_proc.show_normal();

    post_proc.set_phi(feBd,dof);
    post_proc.show_phi();



    for (int pippo=0;pippo<101;pippo++)
    cout << pippo << "," ;
