...
int main(int argc, char** argv) {
  GetPot cl(argc, argv);
  int num_files=cl.vector_variable_size("input_file");
  if (num_files == 0){
    cerr<< "AT LEAST ONE INPUT FILE MUST BE GIVEN" <<endl;
    cerr<< "input_file='file 1 file 2 ...'"<<endl;
    return 1;
  }
  StringVector arg;
  for (int i =0; i != num_files; ++i) arg.push_back(string(cl("input_file","NONE",i)));
 GetPot inputdata(arg);
 inputdata.print();
 //GetPot inputdata(cl("input_file","NONE",0));

 //for (int i=1; i<num_files; ++i) meshdata(cl("input_file",i));
